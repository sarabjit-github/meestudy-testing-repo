...
const AdminStudentChat = () => {
  ...
  const getRecentChats = async () => {
    const res = await api.get("admin-student-conversations/get-all-conversations?page=1&limit=10")
    if (!res.data.length) {
  ...
  async function sendMessage() {
    ...
    try{
      ...
      let a = new FormData()
      a.append("files",fileData[0])
      a.append("senderId","asdfsd")
      a.append("fileText","asdfsd")
      a.append("conversationId","conv id")
      a.append("messageType","File")
    ...
    if (reply) {
      ...
      socket.emit("student_admin_send_message", ({ room_id: studentAdminRoom, data }));
  ...
  useEffect(() => {
    if (socket) {
      ...
      socket.on("student_admin_get_online_user", (data) => {
        ...
        setAllStudents(prev => prev.map(item => {
          if (item?._id === data) {
            return {
              ...item,
              onlineStatus: true
      ...
      <div className={styles.mainChatContainer}>
        <div className={`${styles.leftSideBar} ${toggleClass === true && styles.mobileView}`}>
          ...
          <div className={styles.selectionContainer}>
            ...
            <button
              ...
              className={`${styles.selectionContainerBtn} ${clickedButton === "AllStudent" && styles.toggleselectionContainerBtn}`}
            >
              All Students
            </button>
          </div>
        ...
        <div className={`${styles.rightSideBar} ${toggleClass === false && styles.rightSideMobileView}`}>
          ...
          <div onScroll={handleScroll} className={styles.conversationContainer}>
            {cameraState === true ? (
              <CameraComponent setFileData={setFileData} setCameraState={setCameraState} setAttachmentFiles={setAttachmentFiles} />
            ) : (
              messages.length !== 0 &&
              messages?.map((msg) => {
                return (
